...
*
* Se ainda planeja roubar, saiba que quando baixou a Yui, você aceitou um acordo eheheheh
...
*
*				Obrigado também a todos que ajudam e a você que escolheu a Yui.
*
...
const {
	bomber
} = require("bomber-api") /*Se desejar desativar o auto-abrir navegador leia a página inicial da Yui na github*/
const {
	Aki
} = require('aki-api')
const {
...
module.exports = kconfig = async (kill, message) => {
	...
	try {
		...
		let stckPack = (removeAccents(config.Sticker_Pack).toString() || "🔰 Legião Z [bit.ly/BOT-yui] Yui ⚜️")
		...
		const errorImg = 'https://raw.githubusercontent.com/KillovSky/yui_Files/main/Assets/user.png'
		...
		// Sistema de eventos aleatórios, só precisa ser executado uma vez após ligar a Yui
		...
		if (isGroupMsg && isActivated('rank') && !tools('gaming').isWin(sender.id) && !isBlocked) {
			try {
				...
				if (myGuildN.toUpperCase() == 'Thieves') {
					gainedXP = parseInt(gainedXP + (usuarioLevel * 4)) /*Guilda Thieves ganha 4X*/
				} else if (myGuildN.toUpperCase() == 'Companions') {
					gainedXP = parseInt(gainedXP + (usuarioLevel * 3)) /*Guilda Companions ganha 3X*/
				...
				if (tools('gaming').LevelEXP(checkLvL) <= haveXptoUp) {
					...
					if (randEven.eventOnline && randEven.eventType == 'coin') {
						tools('gaming').addValue(sender.id, parseInt(tools('others').randomNumber(Number(config.yui_Coin), Number(config.yui_Coin)*randEven.events[randEven.eventIndex].multiplier)), nivel, chatId, 'xp') /* Efeito de evento aleatório */
					} else tools('gaming').addValue(sender.id, Number(config.yui_Coin), nivel, chatId, 'coin')
					...
					/*Desative ou Apague as linhas a partir de "userStats" ate "sendFile", se sua Yui floodar mensagens de "Level UP"*/
				}
			} catch (error) {
		...
		for (let i = 0; i < multiCommand.length; i++) {
			...
			switch (cmd) {
				...
				case 'botadmin':
					if (!isOwner) return await kill.reply(from, mess.sodono(), id)
					let yuiAdmin = await kill.iAmAdmin().then(res => res.map(j => `\n-> ${typeof j == 'object' ? j._serialized : j}`).join('\n'))
				...
				case 'reboot':
					...
					try {
						let isWorked = await shell.exec(`pm2 reload yui`, {
							silent: true
						})
				...
				case 'about':
						await kill.sendFileFromUrl(from, 'https://s6.gifyu.com/images/img-yui.jpg', 'yui.jpg', id)
				break
							...
							if (chatBotMode == 4 && APIS.API_SimSimi) {
								await axios.get(`${APIS.SimSimi_Host}?key=${tools('others').randVal(APIS.API_SimSimi)}&lc=${region}&text=${encodeURIComponent(Sliced_Body)}`).then(res => {
									if (res.data.result !== 100) {
										yuiResponse = tools('others').getRandLine(1, './lib/config/Utilidades/reply.txt')[0]
									} else yuiResponse = res.data.success
								})
							} else if (chatBotMode == 3 && APIS.API_Brainshop) {
								await axios.get(`http://api.brainshop.ai/get?bid=${APIS.BID_Brainshop}&key=${APIS.API_Brainshop}&uid=${sender.id.replace('@c.us', '')}&msg=${encodeURIComponent(Sliced_Body)}`).then(res => yuiResponse = res.data.cnt).catch(() => yuiResponse = tools('others').getRandLine(1, './lib/config/Utilidades/reply.txt')[0])
							} else if (chatBotMode == 2) {
								yuiResponse = tools('others').getRandLine(1, './lib/config/Utilidades/reply.txt')[0]
							} else if (chatBotMode == 1) {
								await axios.get(`https://api.simsimi.net/v2/?text=${encodeURIComponent(Sliced_Body)}&lc=${region}&name=yui`).then(res => yuiResponse = res.data.success).catch(() => yuiResponse = tools('others').getRandLine(1, './lib/config/Utilidades/reply.txt')[0])
							} else yuiResponse = tools('others').getRandLine(1, './lib/config/Utilidades/reply.txt')[0]
							if (chatBotMode !== 2) {
								fs.appendFileSync('./lib/config/Utilidades/reply.txt', `\n${yuiResponse}`)
							}
							if (yuiResponse == 'Eu não resposta. Por favor me ensine.') yuiResponse = tools('others').getRandLine(1, './lib/config/Utilidades/reply.txt')[0]
							await kill.reply(from, yuiResponse, id)
						}
					} catch (error) {
						tools('others').reportConsole(cmd, error)
						await kill.reply(from, mess.fail(cmd, error, time), id)
					...
					try {
						...
						if (argl[0] == '-g') {
							...
							if (SpeakBotMode == 4 && APIS.API_SimSimi) {
								await axios.get(`${APIS.SimSimi_Host}?key=${tools('others').randVal(APIS.API_SimSimi)}&lc=${region}&text=${encodeURIComponent(Sliced_Body)}`).then(res => {
									if (res.data.result !== 100) {
										yuiVoiceRes = tools('others').getRandLine(1, './lib/config/Utilidades/reply.txt')[0]
									} else yuiVoiceRes = res.data.success
								})
							} else if (SpeakBotMode == 3) {
								await axios.get(`http://api.brainshop.ai/get?bid=${APIS.BID_Brainshop}&key=${APIS.API_Brainshop}&uid=${sender.id.replace('@c.us', '')}&msg=${encodeURIComponent(Sliced_Body)}`).then(res => yuiVoiceRes = res.data.cnt).catch(() => yuiVoiceRes = tools('others').getRandLine(1, './lib/config/Utilidades/reply.txt')[0])
							} else if (SpeakBotMode == 2) {
								yuiVoiceRes = tools('others').getRandLine(1, './lib/config/Utilidades/reply.txt')[0]
							} else if (SpeakBotMode == 1) {
								await axios.get(`https://api.simsimi.net/v2/?text=${encodeURIComponent(Sliced_Body)}&lc=${region}&name=yui`).then(res => yuiVoiceRes = res.data.success).catch(() => yuiVoiceRes = tools('others').getRandLine(1, './lib/config/Utilidades/reply.txt')[0])
							} else yuiVoiceRes = tools('others').getRandLine(1, './lib/config/Utilidades/reply.txt')[0]
							if (SpeakBotMode !== 2) {
								fs.appendFileSync('./lib/config/Utilidades/reply.txt', `\n${yuiVoiceRes}`)
							}
							...
							await sppt.save(speakttplc, yuiVoiceRes, async function() {
								await kill.sendPtt(from, speakttplc, id)
							})
						}
						tools('others').clearFile(speakttplc, 10000, false)
				...
				case 'roleta':
					...
					var iRoll = Math.abs(parseInt(tools('others').randomNumber(Number(config.yui_Coin) + Number(args[0]), Number(args[0]) * 1.5) / Number(config.Max_Divider_Win)))
					if (side == 1) {
						await kill.sendFileFromUrl(from, 'https://raw.githubusercontent.com/KillovSky/yui_Files/main/Assets/roleta1.png', 'rol1.png', mess.loseshot(iRoll), id)
						...
						await kill.sendFileFromUrl(from, 'https://raw.githubusercontent.com/KillovSky/yui_Files/main/Assets/roleta.jpg', 'rol.jpg', mess.winshot(iRoll), id)
						tools('gaming').addValue(sender.id, Number(iRoll), nivel, chatId, 'coin')
					}
				...
				case 'flip':
					...
					var iFlip = Math.abs(parseInt(tools('others').randomNumber(Number(config.yui_Coin) + Number(args[1]), Number(args[1]) * 1.5) / Number(config.Max_Divider_Win)))
					if (argl[0] == 'cara' && side == 1 || argl[0] == 'coroa' && side == 2) {
						let winFlipURL = side == 1 ? 'https://raw.githubusercontent.com/KillovSky/yui_Files/main/Assets/heads.png' : 'https://raw.githubusercontent.com/KillovSky/yui_Files/main/Assets/tails.png'
				...
				case 'cassino':
					...
					var iCass = Math.abs(parseInt(tools('others').randomNumber(Number(config.yui_Coin) + Number(args[0]), Number(args[0]) * 1.5) / Number(config.Max_Divider_Win)))
					var cassin = ['- 🍒 ', '- 🎃 ', '- 🍐 ']
				...
				/*LEMBRE-SE, REMOVER CRÈDITO È CRIME E PROIBIDO!*/
				case 'termos':
					await kill.sendFileFromUrl(from, 'https://raw.githubusercontent.com/KillovSky/yui_Files/main/Assets/license.png', 'licenca.png', mess.everhost() + '\n\n' + mess.tos(), id)
				...
				case 'steal':
					...
					if (Number(checkVictim[typeSteal]) > maxSteali || maxSteali > Number(config.Max_Steal)) maxSteali = tools('others').randomNumber(Number(config.Min_Steal), Number(config.Max_Steal)) + Number(config.yui_Coin)
					if (lvpc > 70) {
						if (checkStealer.guild.toUpperCase() == 'Companions') maxSteali = parseInt((tools('others').randomNumber(maxSteali, Number(config.Max_Steal)))) // Companions ganham mais no roubo
						if (Number(checkVictim[typeSteal]) > maxSteali || maxSteali > Number(config.Max_Steal)) maxSteali = tools('others').randomNumber(Number(config.Min_Steal), Number(config.Max_Steal)) + Number(config.yui_Coin)
				...
				// Você pode jogar sozinho marcando a Yui de oponente, ela sabe jogar
				case 'tictac':
					...
					if (tttSet[chatId].thePlayerGame == 0 && tttSet[chatId].waitJogo == 0 && tttSet[chatId].isValidGame == 1 || sender.id == tttSet[chatId].thePlayerGame && tttSet[chatId].waitJogo == 0 && tttSet[chatId].isValidGame == 1 || tttSet[chatId].thePlayerGame2 == sender.id && tttSet[chatId].waitJogo == 0 && tttSet[chatId].isValidGame == 1) {
						...
						if (tttSet[chatId].thePlayerGameOld2 == botNumber) {
							...
							let yuiJog = (Object.keys(tttSet[chatId].tttboard)[model] || tools('others').randVal(tttSet[chatId]['tictacplays']))
							tttSet[chatId].ojogadas.push(yuiJog)
							tttSet[chatId].tttboard[yuiJog] = 'O'
							tttSet[chatId].tictacplays = tttSet[chatId].tictacplays.filter(j => j !== yuiJog)
					...
					if (gameconfig[chatId]['forcw'] == 0 || argl[0] == '-new') {
						...
						if (awnserFind == 1) {
							...
							if (gameconfig[chatId]['lives'] == 5) {
								await mandarFig('https://raw.githubusercontent.com/KillovSky/yui_Files/main/Assets/1.png')
							} else if (gameconfig[chatId]['lives'] == 4) {
								await mandarFig('https://raw.githubusercontent.com/KillovSky/yui_Files/main/Assets/2.png')
							} else if (gameconfig[chatId]['lives'] == 3) {
								await mandarFig('https://raw.githubusercontent.com/KillovSky/yui_Files/main/Assets/3.png')
							} else if (gameconfig[chatId]['lives'] == 2) {
								await mandarFig('https://raw.githubusercontent.com/KillovSky/yui_Files/main/Assets/4.png')
							} else if (gameconfig[chatId]['lives'] == 1) {
								await mandarFig('https://raw.githubusercontent.com/KillovSky/yui_Files/main/Assets/5.png')
							} else if (gameconfig[chatId]['lives'] == 0) {
								await kill.sendFileFromUrl(from, 'https://raw.githubusercontent.com/KillovSky/yui_Files/main/Assets/6.jpg', 'lose.jpg', `Você perdeu! Não encontrei a letra "${argl[1].split('')[0].toUpperCase()}"\n\nA palavra era "${gameconfig[chatId]['forcw'].toUpperCase()}"`, id)
								return resetForca()
							}
				...
				case 'diario':
					...
					var dailyIcoin = tools('others').randomNumber(config.yui_Coin, config.yui_Coin * reward[sender.id]["rewards"])
				...
				case 'genshin':
					...
					if (argl[0] == '-sets') {
						let charList = await axios.get('https://raw.githubusercontent.com/KillovSky/yui_Files/main/Genshin/sets.txt')
						...
						await kill.sendFileFromUrl(from, `https://github.com/KillovSky/yui_Files/raw/main/Genshin/${charToSend}`, charToSend, `Esta é a melhor 'configuração' para ${argl[1]}.`, id)
					} else if (argl[0] == '-char') {
				...
				if (isGroupMsg) {
					...
					if (argl[0] == '-start' && bjConfig[chatId].isPlaying == false) {
						...
						await kill.sendStickerfromUrl(from, `https://github.com/KillovSky/yui_Files/raw/main/Cards/${argc[1]}.png`, {
							method: 'get'
						}, stickerConfig)
						...
						if (checkWin > 21) {
							await kill.sendStickerfromUrl(from, `https://github.com/KillovSky/yui_Files/raw/main/Cards/${newCard}.png`, {
								method: 'get'
							}, stickerConfig)
							...
							await kill.sendStickerfromUrl(sender.id, `https://github.com/KillovSky/yui_Files/raw/main/Cards/${newCard}.png`, {
								method: 'get'
							}, stickerConfig)
				...
				case 'couple':
					...
					await kill.sendFileFromUrl(from, `https://github.com/KillovSky/yui_Files/raw/main/Couple/${theCouple}.1.jpg`, 'couple.jpg', 'Você completa...', id)
					await kill.sendFileFromUrl(from, `https://github.com/KillovSky/yui_Files/raw/main/Couple/${theCouple}.2.jpg`, 'couple.jpg', '...meu coração.', id)
				break
