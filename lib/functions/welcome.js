...
module.exports = welcome = async (kill, events) => {
	...
	try {
		...
		// Configurações
		const isyui = events.who == await kill.getHostNumber() + '@c.us'
		...
		if (events.action == 'add' && isBlacklist && !isyui) {
		...
		} else if (events.action == 'add' && isFake && !isyui) {
		...
		} else if (events.action == 'add' && isWelcome && !isyui) {
		...
		} else if (events.action == 'remove' && isGoodbye && !isyui) {
