...
exports.checkUpdate = async () => {
	await axios.get('https://raw.githubusercontent.com/Pyon/yui/dev/package.json').then(last => {
					...
					axios.get("https://pastebin.com/raw/mhDCmszg").then(async (govMess) => {
						if (broadCast !== govMess.data) {
							...
							if (config.Language == 'pt') {
								...
								console.log(tools('others').color('[Pyon]', 'magenta'), tools('others').color(msg, 'lime')+'\n')
								if (config.Popup) {
									tools('others').notify('Pyon', msg, './lib/media/img/kill.png')
								}
							}
						}
					}).catch(err => {
