...
// 'Desativa' o evento aleatório para casos de reinicialização da Yui
...
module.exports = Start_yui = async (kill = new Client()) => {
	try {

		// Caso use múltiplas Yui, impede de fazer tudo de novo
		if (!runOnlyOneTime) {
			runOnlyOneTime = true
			...
			console.log(tools('others').color('[SUPORTE]', 'magenta'), tools('others').color(`https://bit.ly/BOT-yui | Apenas BUG's\n`, 'lime'), tools('others').color(`\n[PRE-START]`, 'magenta'), tools('others').color('Terminei a pré-inicialização, vou esperar até uma primeira mensagem chegar para começar.', 'lime') + '\n')
			if (config.Popup) {
				tools('others').notify('Yui', mylang(config.Language).started((new Date()).getHours()), './lib/media/img/Hello.png')
			}
		}
		...
		let yuiCMD = config.Bot_Commands ? 'onAnyMessage' : 'onMessage'
		kill[yuiCMD](async (message) => {
