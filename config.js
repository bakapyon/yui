...
// Bomber, se desejar desativar o auto-abrir navegador leia a página inicial da Yui na github
...
module.exports = kconfig = async (kill, message) => {
	...
	try {
		...
		const yuiMsgs = await fs.readFileSync('./lib/config/Utilidades/reply.txt').toString().split('\n')
		const chatBotR = yuiMsgs[Math.floor(Math.random() * yuiMsgs.length)].replace('%name$', `${name}`).replace('%battery%', `${lvpc}`)
		...
		// Sistema que permite ignorar comandos de um grupo, caso você já possua um BOT nele e queira deixar a Yui desligada apenas lá, basta ativar
		...
		if (isGroupMsg && isxp && !gaming.isWin(user) && !isBlocked) {
			try {
				...
				if (getReqXP(checkLvL) <= haveXptoUp) {
					await gaming.addValue(user, 1, nivel, 'level');await gaming.addValue(user, Number(config.yui_Coin), nivel, 'coin')
					...
					// Desative ou Apague a "kill.reply" acima se sua Yui floodar mensagens de "Level UP"
				}
		...
		switch(command) {
			...
			case 'horizon':;case 'grupo':
				...
				await kill.sendLinkWithAutoPreview(from, 'https://chat.whatsapp.com/GuIBmndJ5A5AGmw30XIb6k', '\nEsse é o grupo principal da Yui.')
				break
				...
				try {
					if (args[0].toLowerCase() == '-g') {
						...
						if (yui.data.success == 'Limit 50 queries per hour.' || yui.data.success == '' || yui.data.success == null) {
							...
							if (yui.data.success == 'Curta a pagina Gamadas por Bieber no facebook ;)') return await kill.reply(from, 'Oi sua gostosa, como vai?', id)
			...
			case 'speak':
				...
				try {
					if (args[0].toLowerCase() == '-g') {
						...
						const spyui = await axios.get(`http://simsumi.herokuapp.com/api?text=${encodeURIComponent(body.slice(7))}&lang=${region}`)
