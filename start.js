...
const Start_yui = require('./lib/functions/listener')
...
// Cria uma sessão da Yui na Wa-Automate - Ou varias
try {
	if (sesConfig.Session_Multiple_Numbers == true) {
		...
		for (let i = 0; i < sesConfig.Session_ID.length; i++) {
			WA.create(tools('options').options(Start_yui, sesConfig.Session_ID[i], (sesConfig.User_Data_Dir[i] || 'Another_yui_Session'))).then(kill => Start_yui(kill))
		}
	} else {
		WA.create(tools('options').options(Start_yui, sesConfig.Session_ID[0], sesConfig.User_Data_Dir[0])).then(kill => Start_yui(kill))
	}
} catch (error) {
	console.error(error)
}